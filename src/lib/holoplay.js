...
function HoloPlay(scene, camera, renderer, focalPointVector, constantCenter, hiResRender) {
  ...
  function init() {
    ...
    finalRenderCamera = new THREE.OrthographicCamera(
      -window.innerWidth / 2,
      window.innerWidth / 2,
  ...
  function setShaderValues(dpi, pitch, slope, screenH, screenW, center, flipX, flipY, invView) {
    ...
    bufferMat.uniforms.windowInnerY.value =
      window.screenY + boundingRect.top + (window.outerHeight - window.innerHeight)
  ...
  function captureViews(scene, camera) {
    ...
    for (var i = 0; i < numViews; i++) {
      ...
      var tempRight = new THREE.Vector3(
        camera.right.x * offsetX,
        camera.right.y * offsetX,
        camera.right.z * offsetX
      )
  ...
  var buttonDown = new CustomEvent('buttonDown', {
    bubbles: true,
    cancelable: false,
    name: 'none',
    index: -1,
  })
  var buttonPressed = new CustomEvent('buttonPressed', {
    bubbles: true,
    cancelable: false,
    name: 'none',
    index: -1,
  })
  var buttonUp = new CustomEvent('buttonUp', {
    bubbles: true,
    cancelable: false,
    name: 'none',
    index: -1,
